...
const sunSetDown = () => {
  let result = Math.asin((0 - (Math.sin(epsilon) * Math.sin(phi) / (Math.cos(epsilon) * Math.cos(phi)))));
  return Math.round(result * 12 * 60 / Math.PI);
}
const drawLine = () => {
  ...
  ctx.strokeStyle = "red";
  ctx.stroke();
  date = new Date(2018, 0, day.value, 0, 6 * 60 + sunSetDown());
  ...
  sunRiseText.innerHTML = twoCijfers(hour) + ':' + twoCijfers(minute);
}
